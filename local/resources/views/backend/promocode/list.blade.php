                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="card-block">
                                <button class="btn btn-navy rounded-pill btn-sm" type="button" onclick="window.location.href='<?php echo url("backend/promocode/form");?>';">
                                    <i class="ion-plus"></i>
                                </button>
                                <p>&nbsp;</p>
                                <div class="dt-responsive table-responsive">
                                    <table id="simpletable" class="table table-striped table-bordered nowrap">
                                        ...
                                        @endphp
                                        @if(!empty($promocode))
                                            @foreach($promocode as $r)
                                            @php
                                            $i++;
                                            @endphp
                                            <tr>
                                                <td>{{$r->promocode_id}}</td>
                                                <td>{{$r->promocode_name}}</td>
                                                <td>{{$r->promocode_discount}}</td>
                                                <td>{{$r->promocode_type}}</td>
                                                <td>{{$r->promocode_begin_date}}</td>
                                                <td>{{$r->promocode_end_date}}</td>
                                                <td>{{$r->promocode_free_shipping}}</td>
                                                <td>
                                                    @php
                                                    $check = false;     
                                                    @endphp
                                                    @if(!empty($giftset))
                                                        @foreach($giftset as $g)
                                                            @if($r->giftset_id == $g->giftset_id)
                                                                {{ $g->giftset_name }}
                                                                @php
                                                                $check = true;
                                                                @endphp
                                                ...
                                                <td>{{ $r->sub_total_complete }}</td>
                                                <td>{{ $r->amount_limit }}</td>
                                                <td><a href="{{url('backend/promocode/form/'.$r->promocode_id)}}">Edit</a> / <a href="{{url('backend/promocode/delete/'.$r->promocode_id)}}" onclick="return confirm('Confirm Delete');">Delete</a></td>
                                            </tr>
                                            @endforeach
                                        @endif
                                        @if($i == 0)
                                            <tr>
                                                <td colspan="11" align="center">Not Found Data</td>
                                            </tr>
                                        @endif
                                        </tfoot>
                                    </table>
