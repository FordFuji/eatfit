                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="card-block">
                                <input type="button" onclick="window.location.href='<?php echo url("backend/pick_your_plan/form");?>';" value=" Add "><p></p>
                                <div class="dt-responsive table-responsive">
                                    <table id="simpletable" class="table table-striped table-bordered nowrap">
                                        ...
                                        $i = 0;
                                        @endphp
                                        @if(!empty($planYourPlan))
                                            @foreach($planYourPlan as $r)
                                            @php
                                            $i++;
                                            @endphp
                                            <tr>
                                                <td>{{$r->plan_id}}</td>
                                                <td><img src="{{asset($r->plan_image)}}" width="150"></td>
                                                <td>{{$r->plan_name_th}}</td>
                                                <td>{{$r->plan_name_en}}</td>
                                                <td>{{$r->plan_description_th}}</td>
                                                <td>{{$r->plan_description_en}}</td>
                                                <td><a href="{{url('backend/pick_your_plan/form/'.$r->plan_id)}}">Edit</a></td>
                                            </tr>
                                            @endforeach
                                        @endif
                                        @if($i == 0)
